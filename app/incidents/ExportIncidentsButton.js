...
import { getIncidentsExportData } from '@/app/actions/incidents'
import * as XLSX from 'xlsx'

export default function ExportIncidentsButton() {
    const [isExporting, setIsExporting] = useState(false)

    const handleExport = async () => {
        try {
            setIsExporting(true)
            const data = await getIncidentsExportData()
            ...
            const exportData = data.map(item => ({
                ...
                'ชื่อ-นามสุกล': item.staffName || '-',
                'สถานะการบาดเจ็บ': item.status || '-',
                'รายละเอียดเหตุการณ์': item.incidentDetails || '-'
            }))

            const worksheet = XLSX.utils.json_to_sheet(exportData)
            const workbook = XLSX.utils.book_new()
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Staff_Incidents')
            XLSX.writeFile(workbook, `Incidents_Report_${new Date().getTime()}.xlsx`)
        } catch (error) {
