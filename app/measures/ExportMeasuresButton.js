...
import { getMeasuresExportData } from '@/app/actions/measures'
import * as XLSX from 'xlsx'

export default function ExportMeasuresButton({ role }) {
    const [isExporting, setIsExporting] = useState(false)

    const handleExport = async () => {
        try {
            setIsExporting(true)
            const data = await getMeasuresExportData()
            ...
            const exportData = data.map((item) => ({
                'วันที่บันทึก': new Date(item.createdAt).toLocaleString('th-TH'),
                'มาตรการรหัส': item.measureId,
                'สถานะ': item.status ? 'ดำเนินการแล้ว' : 'ยังไม่ทำ',
            ...
            XLSX.writeFile(workbook, `Measures_Report_${new Date().getTime()}.xlsx`)
        } catch (error) {
